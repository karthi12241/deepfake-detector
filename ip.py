...
class GatedFeatureFusion(nn.Module):
    ...
    def forward(self, cnn_features, vit_features):
        ...
        return torch.cat([fused, cnn_proj, vit_proj], dim=1)  # [B, 1536]
...
def predict(model, image: Image.Image, temperature: float = 7.0):
...
temperature = st.sidebar.slider(
    ...
    help="Higher T = more calibrated confidence scores"
)
...
ip_mode = st.sidebar.radio(
    "IP Source",
    ["Simulate IP (Demo)", "Enter IP Manually"],
    help="In real web deployment, IP is captured automatically"
)

if ip_mode == "Simulate IP (Demo)":
...
with col_right:
    ...
    if uploaded_file and analyse_btn:
        with st.spinner("🧠 Running model inference..."):
            label, confidence = predict(model, image, temperature)
